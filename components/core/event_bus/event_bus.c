 ...
 * @brief 事件总线核心实现
 * 
 * 实现事件总线的核心功能，包括事件发布、订阅、处理等。
 * 
 * 架构设计:
 * - 事件队列: 使用 FreeRTOS 队列存储待处理的事件
 * - 订阅者表: 使用固定大小数组存储订阅者信息
 * - 处理任务: 独立任务从队列取出事件并分发给订阅者
 * - 线程安全: 使用互斥锁保护订阅者表的访问
 * 
 * 内存管理:
 * - 事件数据在发布时分配内存，处理完成后释放
 * - 订阅者表使用静态分配，避免碎片化
 * 
 * @author OVS Team
 * @date 2026-09-07
...
event_t* event_bus_create_event(event_type_t type, const void* data, uint16_t data_len) {
    ...
    event->header.timestamp = (uint32_t)(esp_timer_get_time() / 1000);  /* 转换为ms */
...
void event_bus_dispatch_event(const event_t* event) {
    ...
    if (xSemaphoreTake(ctx->mutex, pdMS_TO_TICKS(100)) != pdTRUE) {
        LOGE(TAG, "dispatch_event: failed to take mutex");
        return;
    }
    ...
    for (int i = 0; i < EVENT_BUS_MAX_SUBSCRIBERS; i++) {
        ...
        /* 检查事件类型是否匹配 */
        if (sub->event_type == event->header.type) {
            /* 调用处理函数 */
...
void event_bus_process_task(void* arg) {
    ...
    while (1) {
        /* 从队列中取出事件 */
        if (xQueueReceive(ctx->queue, &node, pdMS_TO_TICKS(EVENT_BUS_POLL_INTERVAL_MS)) == pdTRUE) {
            /* 分发事件 */
...
event_bus_err_t event_bus_init(void) {
    ...
    /* 创建事件队列 */
    ctx->queue = xQueueCreate(EVENT_BUS_QUEUE_SIZE, sizeof(queue_node_t));
    if (ctx->queue == NULL) {
    ...
    LOGI(TAG, "  Task priority: %d", EVENT_BUS_TASK_PRIORITY);
...
event_bus_err_t event_bus_deinit(void) {
    ...
    /* 清空队列 */
    queue_node_t node;
    while (xQueueReceive(ctx->queue, &node, 0) == pdTRUE) {
        event_bus_destroy_event(node.event);
    }
...
event_subscription_t* event_bus_subscribe(event_type_t event_type, 
    ...
    if (xSemaphoreTake(ctx->mutex, pdMS_TO_TICKS(1000)) != pdTRUE) {
        LOGE(TAG, "subscribe: failed to take mutex");
        return NULL;
    }
    ...
    LOGI(TAG, "subscribe: event 0x%08X, id=%lu, total=%d",
         event_type, subscriber->id, ctx->subscriber_count);
...
event_bus_err_t event_bus_unsubscribe(event_subscription_t* subscription) {
    ...
    /* 获取互斥锁 */
    if (xSemaphoreTake(ctx->mutex, pdMS_TO_TICKS(1000)) != pdTRUE) {
        LOGE(TAG, "unsubscribe: failed to take mutex");
    ...
    for (int i = 0; i < EVENT_BUS_MAX_SUBSCRIBERS; i++) {
        ...
        if (sub->active && sub->id == subscription->subscriber_id) {
            ...
            LOGI(TAG, "unsubscribe: id=%lu, remaining=%d",
                 subscription->subscriber_id, ctx->subscriber_count);
            break;
        }
    }
    ...
    if (!found) {
        LOGW(TAG, "unsubscribe: subscriber id=%lu not found", subscription->subscriber_id);
...
event_bus_err_t event_bus_get_status(int* queue_size, int* subscriber_count) {
    ...
    if (queue_size != NULL) {
        *queue_size = (int)uxQueueMessagesWaiting(ctx->queue);
    }
...
event_bus_err_t event_bus_get_stats(uint32_t* events_published, 
    ...
    if (events_published != NULL) {
        *events_published = ctx->stats.events_published;
    }
    
    if (events_processed != NULL) {
        *events_processed = ctx->stats.events_processed;
    }
    
    if (events_dropped != NULL) {
        *events_dropped = ctx->stats.events_dropped;
    }
    
    if (handler_errors != NULL) {
        *handler_errors = ctx->stats.handler_errors;
    }
