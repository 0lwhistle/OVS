...
vfs_err_t vfs_unmount(const char* virtual_path) {
    ...
    mount_point_t* mp = &s_mount_table[idx];
    ...
    if (mp->blockdev_handle) {
        ...
        err = esp_vfs_littlefs_unregister(virtual_path);
        LOGI(TAG, "unregister returned: %s", esp_err_to_name(err));
    }
    if (err != ESP_OK) {
    ...
    /* 释放块设备适配器 */
    if (mp->blockdev_handle) {
        vfs_release_blockdev_adapter(mp->blockdev_handle);
        mp->blockdev_handle = NULL;
    }
