...
用法: python scripts/fs_to_c.py <dist目录> <输出目录>
示例: python scripts/fs_to_c.py web/vue-ui/dist components/modules/web/web_data
"""
...
'''
    ...
    for relpath, fsize in files:
        print(f"  /{relpath} ({fsize} bytes)")

if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python fs_to_c.py <dist_dir> <output_dir>")
        print("Example: python scripts/fs_to_c.py web/vue-ui/dist components/modules/web/web_data")
        sys.exit(1)
