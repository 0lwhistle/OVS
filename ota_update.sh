...
#
# OTA 固件升级脚本（支持断点续传）
# 用法: ./ota_update.sh <esp32-ip> [firmware.bin]
#
# 示例:
#   ./ota_update.sh 192.168.2.154
#   ./ota_update.sh 192.168.2.154 build/ovs_signed.bin
#
# 如果未指定固件路径，默认使用 build/ovs_signed.bin
# 如果签名固件不存在，会自动对 build/ovs.bin 进行签名
#
# 断点续传说明：
#   如果上传中断，重新运行脚本会自动从断点处继续上传。
#   脚本会先查询 ESP32 上的 OTA 进度，然后只上传剩余部分。
...
if [ ! -f "$FIRMWARE" ]; then
    ...
    if [ -f "build/ovs.bin" ]; then
        ...
        echo "Build the firmware first: idf.py build"
        exit 1
    fi
fi
...
python3 tools/sign_firmware.py --verify "$FIRMWARE"
echo ""
...
if [ "$RESUME_OFFSET" -gt 0 ]; then
    ...
    echo -e "  ${YELLOW}curl -X POST -H \"X-Offset: $RESUME_OFFSET\" --data-binary @$FIRMWARE (from offset $RESUME_OFFSET) http://$ESP_IP/ota/update${NC}"
else
    echo -e "  ${YELLOW}curl -X POST --data-binary @$FIRMWARE http://$ESP_IP/ota/update${NC}"
fi
echo ""
...
# ---- 显示结果 ----
echo -e "${CYAN}[4/4] OTA Result${NC}"

if [ "$HTTP_CODE" = "200" ]; then
    echo -e "  HTTP Status: ${GREEN}$HTTP_CODE OK${NC}"
    if [ -f /tmp/ota_response_$$.json ]; then
        echo -e "  Response:    ${GREEN}$(cat /tmp/ota_response_$$.json)${NC}"
    fi
    echo ""
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}  OTA upload successful!${NC}"
    echo -e "${GREEN}  ESP32 is verifying and will reboot.${NC}"
    echo -e "${GREEN}  Check serial console for verification details.${NC}"
    echo -e "${GREEN}========================================${NC}"
else
    echo -e "  HTTP Status: ${RED}$HTTP_CODE ERROR${NC}"
    if [ -f /tmp/ota_response_$$.json ]; then
        echo -e "  Response:    ${RED}$(cat /tmp/ota_response_$$.json)${NC}"
    fi
    echo ""
    echo -e "${RED}========================================${NC}"
